...
*/1 * * * *  cd /home/wenyuan/es_replay/send_data/ && python send_iprobe-dns2es.py >/dev/null 2>&1
"""
import os
import json
import time
...
def make_data(doc_list):
    ...
    for doc in doc_list:
        ...
        # 插入ipv6字段
        doc['dns']['src_ipv6'] = f.ipv6()
        doc['dns']['dst_ipv6'] = f.ipv6()
        # 中文转英文
        src_country = doc['dns']['src_ip'].get('country', None)
        src_province = doc['dns']['src_ip'].get('province', None)
        src_city = doc['dns']['src_ip'].get('city', None)
        src_isp = doc['dns']['src_ip'].get('isp', None)
        if src_country:
            doc['dns']['src_ip']['country'] = pinyin(src_country)
        if src_province:
            doc['dns']['src_ip']['province'] = pinyin(src_province)
        if src_city:
            doc['dns']['src_ip']['city'] = pinyin(src_city)
        if src_isp:
            doc['dns']['src_ip']['isp'] = isp2en(src_isp)

        dst_country = doc['dns']['dst_ip'].get('country', None)
        dst_province = doc['dns']['dst_ip'].get('province', None)
        dst_city = doc['dns']['dst_ip'].get('city', None)
        dst_isp = doc['dns']['dst_ip'].get('isp', None)
        if dst_country:
            doc['dns']['dst_ip']['country'] = pinyin(dst_country)
        if dst_province:
            doc['dns']['dst_ip']['province'] = pinyin(dst_province)
        if dst_city:
            doc['dns']['dst_ip']['city'] = pinyin(dst_city)
        if dst_isp:
            doc['dns']['dst_ip']['isp'] = isp2en(dst_isp)
